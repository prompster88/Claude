...
(async () => {
  ...
  let page; for (const s of scenes) {
    page = await browser.newPage({ viewport: { width: W, height: H }, deviceScaleFactor: 1 });
    page.setDefaultTimeout(300000); page.on('console', m => console.log('[page]', m.text()));
    ...
    require('fs').writeFileSync(`out/scene${s}.png`, Buffer.from(data.split(',')[1], 'base64'));
